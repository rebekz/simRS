...
/**
 * WEB-S-3.2: Returning Patient Quick Check-In (MOBILE-ENHANCED)
 *
 * Key Features:
...
export default function QuickCheckInPage() {
  ...
  // Offline storage for check-in data
  const { isOnline, isSyncing, syncProgress, addToQueue } = useOfflineStorage({
    key: "checkin_queue",
    onOnline: async (items) => {
      // Sync logic would go here in production
      console.log("Syncing check-in items:", items);
    },
  });
  ...
  const handleCheckIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isOnline) {
        addToQueue(checkInData, "/api/checkin", "POST");
      }
      ...
      setQueueTicket({
        ...
        estimatedWait: isOnline ? "15-30 menit" : "15-30 menit (menunggu koneksi)",
      ...
      <div className="min-h-screen bg-gradient-to-br from-teal-50 via-blue-50 to-white flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-teal-500 to-teal-600 p-6 md:p-8 text-center">
            ...
            <h1 className="text-xl md:text-2xl font-bold text-white">Check-In Berhasil!</h1>
            <p className="text-teal-100 mt-1 md:mt-2 text-sm">Pasien terdaftar untuk pemeriksaan</p>
          </div>
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 md:p-4 mb-6">
              <div className="flex items-start gap-2 md:gap-3">
                ...
                <div>
                  <p className="text-xs md:text-sm font-medium text-blue-900">Informasi</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-blue-50 to-white py-4 md:py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-6 md:mb-8">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Quick Check-In</h1>
          <p className="text-gray-600 mt-1 md:mt-2 text-sm">Cek-in pasien lama dengan cepat</p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-4 md:p-6 mb-4 md:mb-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-12 py-2.5 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 text-sm md:text-base"
                autoFocus
              />
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute inset-y-0 right-12 pr-3 flex items-center min-w-[44px] min-h-[44px]"
                >
                  <X className="w-4 h-4 md:w-5 md:h-5 text-gray-400 hover:text-gray-600" />
                </button>
              )}
            </div>
              ...
              <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
                {searchResults.map((patient) => (
                  <button
                    ...
                    className="w-full px-3 md:px-4 py-3 min-h-[48px] hover:bg-gray-50 transition-colors border-b border-gray-200 last:border-b-0 text-left"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-medium text-gray-900 text-sm md:text-base truncate">{patient.name}</p>
                          {patient.bpjsStatus && (
                            <BPJSStatusBadge status={patient.bpjsStatus} size="sm" />
                          )}
                        </div>
                        <p className="text-xs md:text-sm text-gray-600 mt-1">RM: {patient.rmNumber}</p>
                        <div className="flex items-center gap-2 md:gap-4 mt-1 text-xs text-gray-500">
                          <span className="truncate">NIK: {patient.nik}</span>
                          {patient.bpjsNumber && <span className="hidden sm:inline">BPJS: {patient.bpjsNumber}</span>}
                        </div>
                      </div>
                      <ArrowRight className="w-4 h-4 md:w-5 md:h-5 text-gray-400 ml-2 flex-shrink-0" />
                    </div>
                  </button>
                ))}
              </div>
            ...
            {showResults && searchResults.length === 0 && (
              <div className="mt-4 text-center py-6 md:py-8 text-gray-500">
                <User className="w-10 h-10 md:w-12 md:h-12 mx-auto mb-2 text-gray-300" />
                <p className="text-sm">Pasien tidak ditemukan</p>
                <p className="text-xs">Coba kata kunci lain atau daftarkan pasien baru</p>
              </div>
            )}
            ...
            {!searchQuery && (
              <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3 md:p-4">
                <p className="text-xs md:text-sm font-medium text-blue-900 mb-2">Demo Data (Cari salah satu):</p>
        ...
        {selectedPatient && (
          <form onSubmit={handleCheckIn} className="space-y-4 md:space-y-6">
            {/* Patient Info Card */}
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="bg-gradient-to-r from-teal-500 to-teal-600 px-4 md:px-6 py-3 md:py-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-teal-100 hover:text-white text-xs md:text-sm"
                >
                  Ganti Pasien
                </button>
              </div>
              ...
              <div className="p-4 md:p-6">
                <div className="flex items-start gap-3 md:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base md:text-lg font-semibold text-gray-900 truncate">{selectedPatient.name}</h3>
                    <div className="grid grid-cols-2 gap-2 mt-2 text-xs md:text-sm">
                      <div>
                        <span className="text-gray-500">No. RM:</span>
                        <span className="ml-1 md:ml-2 font-medium text-gray-900">{selectedPatient.rmNumber}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Usia:</span>
                        <span className="ml-1 md:ml-2 font-medium text-gray-900">{selectedPatient.age} tahun</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Jenis Kelamin:</span>
                        <span className="ml-1 md:ml-2 font-medium text-gray-900">{selectedPatient.gender === "male" ? "Laki-laki" : "Perempuan"}</span>
                      </div>
                      <div>
                        <span className="text-gray-500">Kunjungan:</span>
                        <span className="ml-1 md:ml-2 font-medium text-gray-900">{selectedPatient.lastVisit}</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t border-gray-200">
                    <div className="flex items-center justify-between mb-2 md:mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs md:text-sm font-medium text-gray-700">Status BPJS</span>
                      </div>
                      {checkingBPJS && (
                    ...
                    {bpjsData && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-2 md:p-3">
                        <div className="flex items-center gap-1 md:gap-2 text-green-700">
                          ...
                          <span className="text-xs md:text-sm font-medium">BPJS Aktif</span>
                        </div>
              ...
              <div className="p-4 md:p-6 space-y-3 md:space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 md:px-4 py-2.5 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 text-sm md:text-base"
                  />
                </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-3">
              ...
              <button
                ...
                className="w-full sm:w-auto flex-1 px-4 py-3 min-h-[44px] bg-gradient-to-r from-teal-600 to-teal-700 text-white rounded-lg hover:from-teal-700 hover:to-teal-800 font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm"
              >
                {loading ? (
                  <>
